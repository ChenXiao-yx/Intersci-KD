...
**知识图谱概念类型**（§9 工具映射引用，配置层不直接加载；落表计分前由 `normalize_type` 按 venue/DOI 归一化到上表标准类型）：
...
**类型匹配细则**（自原精简表节并入）：关键词大小写不敏感，中英文混合匹配（同时接受「RCT」和「随机对照试验」）；一条证据同时命中多个类型关键词时，**取权重最高的类型**作为最终类型，不做多重加权；模棱两可时按 §1 最匹配的子类型取值，不要套用大类。
...
**四类指标判定细则**（自原乘数细则节并入，"命中即计数"原则——摘要或正文出现任何一类关键词即命中，不要求完整统计报告、不要求数值显著）：
- 样本量描述：n= / N= / 患者数 / 受试者人数 / 样本规模 等明确数量描述；
- 统计指标：p-value / p<0.05 / 置信区间 CI / 95%CI / 显著差异 / 效应量 / OR 值 / HR 值 等；
- 明确结果结论：结果显示 / 结论表明 / 显著优于 / 非劣效于 / 达到主要终点 等；
- 方法学步骤：采用 RCT 设计 / 双盲对照 / 前瞻性队列 / 实验流程 / 统计分析方法 等。
...
| red（红） | 总分 < 4.0 | 弱 | 🔴 |
**年份取值边界规则**（自原衰减细则节并入）：年份基准为当前系统日期的公历年份；未提供年份不衰减但需在有效性状态列注明「年份缺失，衰减未执行」；online first / epub ahead of print 优先用在线发表日期；年份范围为区间（如 2018-2020）取中值；年代描述（如 2010 年代）取该年代最后一年（偏保守策略）。

**等级处理策略**（自原等级阈值节并入）：green 可支撑大部分【确证】判断、用于正式决策；yellow 支撑中等置信度判断、建议追加定向检索或专家咨询（这是最常见的中间状态，AI 不必刻意追求 green）；red 仅支撑【推断】级判断，建议用户提供更多材料后重新蒸馏。总分等级是 §14 三选一矩阵的输入之一。
...
**已知限制与回滚条件（第五点评风险 4，诚实留档）**：上述域级门槛是**基于证据形态的原理推演，不是实测标定**——CHANGELOG 中没有"2.5 确实误杀了合理 AI 方向"的真实案例记录。它确实降低了"结论可信"的门槛（全局 2.5 要求"有人做过综合"，AI 域 1.5 只要求"有一篇会议论文"）。回滚条件：等 live/freeform 真实数据产生后，若发现 AI=1.5 让**本应 insufficient 的方向**（纯会议论文堆量、无复现验证、无外部验证）系统性输出 yellow/green，应把门槛调回 2.5——**宁可多返回 insufficient，也不降低结论可信度**；核心证据门槛是本系统的诚信底线，不为覆盖率妥协。
