...
扫描以下单一事实源与文档之间的一致性：
- SKILL.md frontmatter 版本 ↔ pyproject.toml version（P0-1 版本号对账）
...
def _check_version_consistency(issues):
    """P0-1：校验 SKILL.md frontmatter 版本 == pyproject.toml version。

    SKILL.md 的 version 带 "-skill" 后缀（如 "4.6.0-skill"），
    提取时只取数字段，与 pyproject 的纯版本号对齐比较。
    """
...
def _check_tool_count_consistency(issues):
    ...
    文档中 "N 个 SCP" 的 N 必须是 TOOL_COUNT（工具数）或 ENDPOINT_COUNT（端点数）；
    ...
    for fname in ("SKILL.md", "README.md"):
        ...
        # 检测 "N 个 SCP" 声明（含 "N个SCP" 紧凑形式）
        for m in re.finditer(r'(\d+)\s*个\s*SCP', text):
            declared = int(m.group(1))
            if declared not in (TOOL_COUNT, ENDPOINT_COUNT):
                issues.append(
                    f"{fname} 声明 {declared} 个 SCP 工具，"
